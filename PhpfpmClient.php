...
class PhpfpmClient
{
	...
	private function connect()
	{
		if (!$this->_sock) {
			...
			if (!$this->_sock) {
				throw new \Exception('Unable to connect to FastCGI application: ' . $errstr, 500);//patch 500
			}

			if (!$this->set_ms_timeout($this->_readWriteTimeout)) {
				throw new \Exception('Unable to set timeout on socket',500);
			}
		}
	}
	...
	public function getValues(array $requestedInfo)
	{
		...
		if ($resp['type'] == self::GET_VALUES_RESULT) {
			...
			throw new \Exception('Unexpected response type, expecting GET_VALUES_RESULT',500);
		}
	}
	...
	public function async_request(array $params, $stdin)
	{
		...
		if (fwrite($this->_sock, $request) === false || fflush($this->_sock) === false) {
			...
			if ($info['timed_out']) {
				throw new TimedOutException('Write timed out',500);
			}

			// Broken pipe, tear down so future requests might succeed
			fclose($this->_sock);
			throw new \Exception('Failed to write request to socket',500);
		}
	...
	public function wait_for_response($requestId, $timeoutMs = 0) {
		...
		if (!isset($this->_requests[$requestId])) {
			throw new \Exception('Invalid request id given',500);
		}
		...
		if (!is_array($resp)) {
			...
			if ($info['unread_bytes'] == 0
				&& $info['blocked']
				&& $info['eof']) {
				throw new ForbiddenException('Not in white list. Check listen.allowed_clients.',500);
			}

			throw new \Exception('Read failed',500);
		}
		...
		case self::CANT_MPX_CONN:
			throw new \Exception('This app can\'t multiplex [CANT_MPX_CONN]',500);
			break;
		case self::OVERLOADED:
			throw new \Exception('New request rejected; too busy [OVERLOADED]',500);
			break;
		case self::UNKNOWN_ROLE:
			throw new \Exception('Role value not known [UNKNOWN_ROLE]',500);
			break;
